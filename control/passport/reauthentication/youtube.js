...
module.exports = function(token){
    ...
    request.post({url: url, form: postBody}, function(err, res, body){
        ...
        User.update({'youtube.refresh_token': token},{$set:{'youtube.access_token': JSON.parse(res.body)["access_token"], 'youtube.expires_in': new Date().getTime() + (parseInt(JSON.parse(res.body)["expires_in"])*1000)}}, {multi: false}, function(err, status){
            if(err){
                console.log(err)
            }else{
