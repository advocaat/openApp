...
//                 counter += 1;
//                 for (var i = 0; i < JSON.parse(res.body)["rows"].length; ++i) {
//                     console.log("row "+JSON.parse(res.body)["rows"][i]);
...
//
// }
// function doThing(songIds,callback){
        ...
        request(url,{clientID:  config.youtube.clientID }, function(err, res, body){
            ...
            if(err){
                ...
                for(var j = 0; j < songIds.length; j++ ){
                    // songIds.forEach(function(id){
                    functions.retreiveStats(songIds[j]);
